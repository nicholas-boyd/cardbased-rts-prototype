...
public class ItemTests : MonoBehaviour
{
    ...
    GameObject CreateActor(string title)
    {
        ...
        s[StatTypes.CON] = UnityEngine.Random.Range(30, 50);
        return actor;
    }
    #endregion
    #region Private
    void CreateItems()
    {
        ...
        inventory.Add(CreateEquippableItem("Glove", StatTypes.STR, 10, EquipSlots.Hands));
        inventory.Add(CreateEquippableItem("Boots", StatTypes.DEX, 15, (EquipSlots.Feet)));
        inventory.Add(CreateEquippableItem("Hat", StatTypes.CON, 10, EquipSlots.Head));
    }
    ...
    void Attack(GameObject attacker, GameObject defender)
    {
        ...
        int damage = Mathf.FloorToInt((s1[StatTypes.STR] * 4 - s2[StatTypes.CON] * 2) * UnityEngine.Random.Range(0.9f, 1.1f));
    ...
    void LogToConsole(GameObject actor)
    {
        ...
        string message = string.Format("Name:{0} HP:{1}/{2} ATK:{3} DEF:{4}", actor.name, s[StatTypes.HP], s[StatTypes.MHP], s[StatTypes.STR], s[StatTypes.CON]);
        Debug.Log(message);
    }
    #endregion
}